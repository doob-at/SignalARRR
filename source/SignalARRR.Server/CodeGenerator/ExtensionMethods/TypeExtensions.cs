...
namespace doob.SignalARRR.Server.CodeGenerator.ExtensionMethods {
    public static class TypeExtensions {
        ...
        public static (bool IsStreamingType, doob.SignalARRR.Server.CodeGenerator.StreamingType StreamingType) IsStreamingType(this Type type) {


            if (type.IsGenericTypeOf(typeof(IObservable<>))) {
                return (true, doob.SignalARRR.Server.CodeGenerator.StreamingType.Observable);
            }

            if (type.IsGenericTypeOf(typeof(ChannelReader<>))) {
                return (true, doob.SignalARRR.Server.CodeGenerator.StreamingType.ChannelReader);
            }

            if (type.IsGenericTypeOf(typeof(IAsyncEnumerable<>))) {
                return (true, doob.SignalARRR.Server.CodeGenerator.StreamingType.AsyncEnumerable);
            }

            return (false, doob.SignalARRR.Server.CodeGenerator.StreamingType.None);
